...
class TransitGateway extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    ssmPrivateSG.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.allIcmp());
    ...
    //   Gateway and the simulated customer on-prem environment is not required)
    // ------------------------------------------------------------------------------------
    ...
    const ssmPrivateSG2 = new ec2.SecurityGroup(this, "ssm-private-sg-2", {
      vpc: customerVpc,
      securityGroupName: "tgw-poc-ssm-private-sg-2",
      description: "TGW Demo - SSM Private Security Group 2",
      allowAllOutbound: true,
    });
    ssmPrivateSG2.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.allIcmp());

    const demoInstance2 = new ec2.CfnInstance(this, "demo-instance-2", {
      subnetId: customerVpc.publicSubnets[0].subnetId,
      imageId: latestLinuxAMI.getImage(this).imageId,
      instanceType: "t4g.nano",
      iamInstanceProfile: new iam.CfnInstanceProfile(
        this,
      ...
      securityGroupIds: [ssmPrivateSG2.securityGroupId],
    });
