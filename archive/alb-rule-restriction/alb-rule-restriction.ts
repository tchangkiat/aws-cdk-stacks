...
export class ALBRuleRestriction extends Stack {
    constructor(
        ...
        vpc.privateSubnets.forEach(
            ({ routeTable: { routeTableId } }, index) => {
                new ec2.CfnRoute(
                    this,
                    id +
                        "-vpc1-peering-connection-private-subnet-route-" +
                        index,
                    {
                        destinationCidrBlock: "11.0.0.0/16",
                        routeTableId,
        ...
        vpc2.publicSubnets.forEach(
            ({ routeTable: { routeTableId } }, index) => {
                new ec2.CfnRoute(
                    this,
                    id +
                        "-vpc2-peering-connection-public-subnet-route-" +
                        index,
                    {
        ...
        vpc2.privateSubnets.forEach(
            ({ routeTable: { routeTableId } }, index) => {
                new ec2.CfnRoute(
                    this,
                    id +
                        "-vpc2-peering-connection-private-subnet-route-" +
                        index,
                    {
                        destinationCidrBlock: "10.0.0.0/16",
                        routeTableId,
        ...
        const ec2AutoScalingGroup = new AutoScalingGroup(
            this,
            id + "-ec2-asg",
            {
